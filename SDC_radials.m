...
for network_idx=1:numNetworks
    ...
    for station_idx=1:numStations
        ...
        % v2.1.1
        [rnA_err, datasetName, datasetSize, startDate, endDate, sdnLocalCDIid] = radial_netCDF_aggregation_v211(network_data(network_idx,:),network_columnNames,station_data(station_idx,:),station_columnNames,timeSpan);
        if(rnA_err==0)
            disp(['[' datestr(now) '] - - ' datasetName ' radial v2.1.1 dataset successfully created and stored.']);
        end
