%% radial_netCDF_aggregation_v211.m
...
% The v211 version creates aggregated dataset according the the v2.1.1
...
function [rnA_err, ncFileNoPath, ncFilesize, tStart, tEnd, dataID] = radial_netCDF_aggregation_v211(networkData,networkFields,stationData,stationFields,timeSpan)

disp(['[' datestr(now) '] - - ' 'radial_netCDF_aggregation_v211.m started.']);
...
try
    ...
    if (~isempty(nc.time))
        ...
        nc.scdr = ncread(SDC_OpenDAP_data_url,'SCDR',[1,1,min(iTime)],[length(site_code),maxSite_dim,length(iTime)]);
