...
try
    ...
    if ~(strcmp(stationData(last_calibration_dateIndex),'null'))
        last_calibration_date = datenum(stationData(last_calibration_dateIndex));
        last_calibration_date = last_calibration_date(last_calibration_date~=0);
        lastPatternStr = [datestr(last_calibration_date, 'yyyy-mm-dd') 'T' datestr(last_calibration_date, 'HH:MM:SS') 'Z'];
    else
        lastPatternStr = 'N/A';
    end
catch err
...
try
    ...
    xlink = ['<sdn_reference xlink:href="http://seadatanet.maris2.nl/v_cdi_v3/print_xml.asp?edmo=134&identifier="' dataID '" xlink:role="isDescribedBy" xlink:type="SDN:L23::CDI"/>'];    
catch err
    disp(['[' datestr(now) '] - - ERROR in ' mfilename ' -> ' err.message]);
    rnA_err = 1;
end
...
%     ncwriteatt(ncfile, '/', 'featureType', char('surface'));
    ...
    ncwriteatt(ncfile, '/', 'software_name', char('HFR_Node__SDC_Processing'));
